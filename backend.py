...
bucket3 = client3.get_bucket('title_title')
...
index_title = pickle.loads(bucket3.get_blob('title_index.pkl').download_as_string())
index_title.bucket_name = "title_title_title"

index_title_stem = pickle.loads(bucket3.get_blob('title_stem_index.pkl').download_as_string())
index_title_stem.bucket_name = "title_bin_stem"

index_body = pickle.loads(bucket3.get_blob('body_index.pkl').download_as_string())
index_body.bucket_name = "body_bim_ny"

index_body_stem = pickle.loads(bucket3.get_blob('body_stem_index.pkl').download_as_string())
index_body_stem.bucket_name = "body_bin_stem"

index_anchor = pickle.loads(bucket3.get_blob('anchor_index.pkl').download_as_string())
index_anchor.bucket_name = "anchor_bin_ny"
...
def preprocess_query(  query_as_string):
    ...
    tokens = [token.group() for token in RE_WORD.finditer(query_as_string.lower()) if token.group() not in all_stopwords]
...
def tf_idf_and_cosine(query_tokens, index):
    ...
    posting_lists = {term: index.read_posting_list(term, index.df[term]) for term in np.unique(query_tokens) if term in index.df}
    # Create lists of document ids and term frequencies at once
    candidates_dict = {term: dict(posting_list) for term, posting_list in posting_lists.items()}
    #Extract the document ids from the candidates_dict
    candi = {doc_id for term_freq in candidates_dict.values() for doc_id in term_freq}
    for doc in candi:
        tf_ids_score = np.zeros(n)
        for i,w in enumerate(query_tokens):
            if (w in index.df) and (doc in candidates_dict[w]):
                tf_ids_score[i] = (candidates_dict[w][doc]/index.DL[doc])*np.log2(6348911 / index.df[w])
            else: tf_ids_score[i] = 0
        # use numpy dot product
        answer[doc] = np.dot(tf_ids_score, query_lst) / (np.linalg.norm(query_lst) * id2tf[doc] )
    return answer
...
def merge_results_pr( scores1, scores2, pr, w1=0.33, w2=0.33, w3=0.33, N=40):
    ...
    merged = [(k, (scores1.get(k, 0) * w1) + (scores2.get(k, 0) * w2) + ((pr.get(k, 0)/pr_highest) * w3)) for k in set(scores1) | set(scores2)]
...
def merge_results_pr_withtitle( scores1, pr, w1=0.666, w3=0.33, N=40):
    # Union all docs and add weights, then sort
    pr_highest = max(pr.values())
    merged = [(k, (scores1.get(k, 0) * w1)  + ((pr.get(k, 0)/pr_highest) * w3)) for k in scores1 ]
    return sorted(merged, key=lambda x: x[1], reverse=True)[:N]
...
def merge(score_title_res, score_body):
    if len(score_title_res) != 0:
        ...
        score_title = {doc_id: score / max_title for doc_id, score in score_title_res.items()}
        result = merge_results_pr(score_body, score_title, pr, 0.3333, 0.3333, 0.3333)
    else:
...
def get_sorted_docs( dict):
    result = sorted([(doc_id, score) for doc_id, score in dict.items()], key=lambda x: x[1], reverse=True)
    return result
