...
def search_body():
    ...
    query_list = backend.preprocess_query(query)
    cos_sim = backend.tf_idf_and_cosine(query_list, backend.index_body)
    top_n = backend.get_sorted_docs(cos_sim)
    if len(top_n) < 100:
        result = top_n
    else:
        result = top_n[:100]
...
def search_anchor():
    ...
    query_lst = backend.preprocess_query(query)
    anchor = backend.search__anchor(query_lst, backend.index_anchor)
    result = backend.get_sorted_docs(anchor)
    res = backend.id_title_dict_anchor(result)
    return res
...
def get_pagerank():
    ...
    for docID in wiki_ids:
        if docID in backend.pr:
            item = float(backend.pr[docID])
            res.append(item)
...
def get_pageview():
    ...
    # BEGIN SOLUTION
    for docID in wiki_ids:
        if docID in backend.pv:
            item = backend.pv[docID]
            res.append(item)
